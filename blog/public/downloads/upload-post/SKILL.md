...
metadata: {"openclaw":{"requires":{"env":["UPLOAD_POST_API_KEY","UPLOAD_POST_PROFILE"]},"primaryEnv":"UPLOAD_POST_API_KEY","envVars":[{"name":"UPLOAD_POST_API_KEY","required":true,"description":"API key used to authenticate requests to Upload-Post."},{"name":"UPLOAD_POST_PROFILE","required":true,"description":"Upload-Post Profile name sent as the user parameter."}]}}
---
...
This is an OpenClaw-specific adaptation of
[Upload-Post/upload-post-skill](https://github.com/Upload-Post/upload-post-skill),
whose README declares the project under the MIT license. It was synchronized
with upstream on 2026-08-30. Use it to replace `SKILL.md` in an existing
installation and keep the original `references/` directory.
...
> **Credentials (this deployment).** The API key lives in the environment
> variable `$UPLOAD_POST_API_KEY` and the profile name in
> `$UPLOAD_POST_PROFILE`. Always let the shell expand them inside **double**
> quotes — single quotes do not expand variables. Never print their values,
> never echo them into logs, and never include them in a reply to the user.
> If a command fails with `401`, verify the variable is set with
> `[ -n "$UPLOAD_POST_API_KEY" ] && echo set` rather than printing it.
...
Run this before a batch of uploads to confirm the key is live:
...
- `timezone`: IANA timezone (e.g., "America/Merida")
- `async_upload`: Set `true` for background processing
- `first_comment`: Auto-post first comment

Local media must live inside a mounted volume the agent can reach
(e.g. `/home/node/.openclaw/workspace/`). Paths outside the container do not exist.
...
Note the **double** quotes on `-d`: the JSON body needs variable expansion, so
single quotes would send the literal string `$UPLOAD_POST_PROFILE`.
...
```bash
curl "https://api.upload-post.com/api/analytics/$UPLOAD_POST_PROFILE?platforms=instagram,tiktok" \
  -H "Authorization: Apikey $UPLOAD_POST_API_KEY"
```
...
```bash
curl "https://api.upload-post.com/api/uploadposts/reddit/detailed-posts?profile_username=$UPLOAD_POST_PROFILE" \
  -H "Authorization: Apikey $UPLOAD_POST_API_KEY"
```
...
A `400` on an otherwise well-formed request usually means the `user` value is
not a real Profile name. A `401` means `$UPLOAD_POST_API_KEY` did not reach the
process — check the container environment before touching anything else.
